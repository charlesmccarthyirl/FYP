...
When Supposing a single Case for addition to the Case Base, irrespective of the Label we choose to suppose, the Case Base will classify it with only one Label. E.g. in \ref{fig:NNsCorrect}, irrespective of if we Suppose Cn to be a triangle, or a circle, the Case Base will still classify it a circle. Thus, the Reachability Set of Cn will only get populated in one instance – when we Suppose Cn to be the same Label as the Case Base classifies it.
...
\subsection{CompStrat 3 – NCATMax: New Case Coverage Added Total Maximisation}

\subsection{CompStrat 4 – NCATMaxDMin: New Case Coverage Added Total Maximisation with Deviation Minimization}
