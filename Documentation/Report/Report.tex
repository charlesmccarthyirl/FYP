...
Hu defines an Instance's \emph{Density} as the sum of similarities within an Instance's neighbourhood.
\[
density(x_{i})=\underset{x_{r}\in N_{i}}{\sum}sim(x_{i},x_{r})
\]

The Neighbourhood of an Instance she defines as:
\[
...
Diversity measures how different an Instance is from everything already in the Case Base (all the Labelled Instances).
...
Hu's rough approach is to build a Candidate set of instances which one might want to add to the Case Base. This Candidate Set is based roughly on the Instances in that are furthest away from all the Cases in the Case Base. From these Candidates, one then chooses ones that are from dense regions of the state space.
...
In \citet{Delany2005} - Delany proposes five confidence measures. Hu chooses the three of these which she determines experimentally to have the least correlation\footnote{The validity / appropriateness of this approach will not be considered further though - since this particular paper - and the piece of interest to us - is that of aggregating confidence measures. Better descriptions, explanations and justifications of the measures can be found in \cite{Delany2005}}.


For the three, the following metrics are needed\footnote{Like and Unlike are compared to the Instance's predicted class by the kNN Classifier, since obviously, the actual label of the Instance is unknown.}:
\begin{itemize} 
...
This represents the ratio of the Similarity values between an Instance's k Nearest Like Neighbours, and k Nearest Unlike Neighbours. Intuitively, a higher ratio is better, as this would indicate that the classifier should be certain about its classification, since the cases with the same label as the predicted label are relatively a lot closer than the ones of a different label.
