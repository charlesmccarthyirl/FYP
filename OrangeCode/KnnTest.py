...
class Test(unittest.TestCase):
    def setUp(self):
        ...
        self.knn = BreadAndButter.KNN(data, dist_meas, true_oracle, possible_classes)

    def test_get_probabilities(self):
        probs = self.knn.get_probabilities((1, ''), k=2)
        self.assertItemsEqual([('blue', 1.0)], probs)
        
    def test_find_nearest(self):
        nearest = self.knn.find_nearest((1, ''), k=2)
        self.assertItemsEqual([(1, 'blue'), (2, 'blue')], nearest)
    
    def test_classify(self):
        classification = self.knn.classifiy((1, ''), k=2)
        self.assertEqual("blue", classification)
        
    def test_get_probabilities1(self):
        probs = self.knn.get_probabilities((3.5, ''), k=2)
        self.assertItemsEqual([('blue', 0.5), ('red', 0.5) ], probs)
        
    def test_find_nearest1(self):
        nearest = self.knn.find_nearest((3.5, ''), k=2)
    ...
    def test_classify1(self):
        ...
        classification = self.knn.classifiy((3.5, ''), k=2)
    ...
    def test_find_nearest2(self):
        nearest = self.knn.find_nearest((1.5, ''), k=1)
